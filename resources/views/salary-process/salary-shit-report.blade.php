    ...
    $(document).ready(function () {
        ...
        `;
            ...
            data.employee_salary_data.forEach((item, index) => {
                ...
                if (item.bank_distributions && item.bank_distributions.length > 0) {
                    bankAmountHtml = item.bank_distributions.map(d => `${d.bank_name}: ${nf(d.amount)}`).join('<br>');
                    accountNumberHtml = item.bank_distributions.map(d => `${d.bank_name}: ${d.account_number || 'N/A'}`).join('<br>');
                } else {
                    bankAmountHtml = nf(bankAmount);
                    accountNumberHtml = item.account_number || '';
                }
                ...
                <tr>
                    ...
                    <td>${nf(otAmount)}</td><td>${nf(netPayable)}</td><td>${bankAmountHtml}</td>
                    <td>${nf(cashAmount)}</td><td>${accountNumberHtml}</td><td>${item.remarks || ''}</td>
                </tr>`;
            });
