...
$(document).ready(function() {
    ...
    $('#saveData').on('click', function(e) {
        ...
        if (!$('#employeeId').val()) return validate('Please select an employee.');
        if (!$('#entryDate').val()) return validate('Please select an entry date.');
        if (!$('#effectiveDate').val()) return validate('Please select an effective date.');
        if (!$('#gross').val()) return validate('Please enter a gross amount.');
        ...
        success: function(data) {
            ...
            data.forEach(function(item) {
                var row = `<tr>
                    ...
                    <td>${item.user_info.effective_date || ''}</td>
                    <td>${item.user_info.entry_date || ''}</td>
                    <td>${item.user_info.gross || ''}</td>`;
