...
Class BackupController extends Controller{
	...
	public function index(){
		...
		return view('backup_log.index',compact('col_heads','table_info'));
	}
	...
	public function store(Request $request){
		if($request->has('delete_old_backup')){
			Backup::truncate();
			File::cleanDirectory(config('constants.upload_path.backup'));
		}
        ...
        if(!File::exists(config('constants.upload_path.backup')))
        File::makeDirectory(config('constants.upload_path.backup'));
	...
	public function destroy(Backup $backup,Request $request){
		...
		if(File::exists(config('constants.upload_path.backup').$backup->file))
			File::delete(config('constants.upload_path.backup').$backup->file);
        $backup->delete();
