...
class EmpoloyeeCreate extends Controller
{
    ...
    {
        try {
            $oldUsers = DB::table('tbluser')->where('BranchName',"KASUNDI RESTORA LTD (ICDDRB)")->get();
            ...
            return response()->json([
                'status' => 'success',
                'message' => $migrated . "user(s) migrated successfully to Kasundi Restora Ltd. (ICDDRB Branch)!"
            ]);
