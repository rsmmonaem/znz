...
Class TemplateController extends Controller{
	...
	public function index(Template $template){
		...
		foreach(config('template') as $key => $value){
			$find_template = $templates->whereLoose('category',$key)->count();
			if($find_template == 0){
				$body = File::get(base_path().'/resources/views/'.$value['path'].'.blade.php');
				$insert[] = array('is_default' => 1,'name' => Helper::toWord($key),'subject' => $value['default_subject'], 'body' => $body,'category' => $key);
			}
		}
		...
		$assets = ['rte'];
		return view('template.index',compact('col_heads','table_info','assets','category'));
	}
	...
	public function lists(Request $request){
        ...
        foreach($templates as $template){
			...
			$rows[] = array(
				...
				'</div>',
				$template->name,
				Helper::toWord($template->category),
				$template->subject
				);
        }
        $list['aaData'] = $rows;
