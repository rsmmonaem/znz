  ...
  public function reportPost(Request $request)
  {
    $data = IncrementsPromotions::leftJoin('users', 'increments_promotions.employee_id', '=', 'users.id')
      ...
      ->when($request->month && $request->financialYear, function ($query) use ($request) {
        return $query->whereMonth('increments_promotions.entry_date', '=', $request->month)
          ->whereYear('increments_promotions.entry_date', '=', $request->financialYear);
      })
      ...
      ->select(
        ...
        'increments_promotions.amount as promotedAmount',
        'increments_promotions.designation as promotedDesignation',
        'increments_promotions.id',
        'profile.date_of_joining',
        'increments_promotions.old_amount',
        'increments_promotions.promotion',
        'increments_promotions.increment'
      )
      ->get();
